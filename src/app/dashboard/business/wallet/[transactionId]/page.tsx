    ...
    <div className='flex items-center justify-between py-3 text-sm border-b last:border-0'>
      ...
      <div className='flex items-center font-medium'>
        <span
          className={`${
            mono ? 'font-mono' : ''
          } truncate max-w-[200px] md:max-w-[300px]`}
        >
          {value}
        </span>
...
export default function BusinessWalletTransactionDetailPage() {
  ...
  const externalTransaction = useWalletExternalTransactionById(
    transactionType === 'external' ? transactionId : null
  );
  const internalTransaction = useWalletInternalTransactionById(
    transactionType === 'internal' ? transactionId : null
  );
  ...
  const isLoading =
    transactionType === 'internal'
      ? internalTransaction.isLoading
      : externalTransaction.isLoading;
  ...
  if (!transaction)
    return (
      <div className='flex flex-col items-center justify-center h-[50vh] text-muted-foreground'>
        <AlertCircle className='h-10 w-10 mb-2' /> Transaction not found
      </div>
    );
  ...
  if (transaction.proofOfTransferImages) {
    if (Array.isArray(transaction.proofOfTransferImages)) {
      transaction.proofOfTransferImages.forEach((p: string) =>
        proofImages.push(p)
      );
    } else if (typeof transaction.proofOfTransferImages === 'object') {
      Object.values(transaction.proofOfTransferImages).forEach((p: any) =>
        proofImages.push(p as string)
      );
    }
  }
  ...
  return (
    <div className='max-w-8xl mx-auto space-y-6 pb-20'>
      ...
      {/* Header Section */}
      <div
        className={`rounded-xl border p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 ${statusConfig.color}`}
      >
        <div className='flex items-start gap-4'>
          <div
            className={`p-3 rounded-full bg-white/50 dark:bg-black/20 backdrop-blur-sm shadow-sm`}
          >
            <StatusIcon className='h-8 w-8' />
          </div>
          <div>
            <h1 className='text-lg font-semibold opacity-90'>
              {isWithdraw ? 'Withdrawal' : 'Transaction'} {statusConfig.label}
            </h1>
        ...
        <div className='text-right'>
          <p className='text-sm font-medium opacity-75 mb-1 uppercase tracking-wide'>
            Total Amount
          </p>
            ...
            <Card className='border-orange-200 bg-orange-50/50 dark:border-orange-900 dark:bg-orange-950/10'>
              <CardContent className='p-6 flex items-center justify-between'>
                <div className='text-sm text-orange-800 dark:text-orange-300'>
                  <span className='font-semibold'>Pending Approval:</span> You
                  can cancel this request while it is still pending.
                </div>
                <Button
                  size='sm'
                  variant='destructive'
                    ...
                    .map((event: any, index: number, arr: any[]) => {
                      ...
                      return (
                        <div key={event.id || index} className='relative'>
                          <span
                            className={`absolute -left-[31px] top-1 h-4 w-4 rounded-full border-2 bg-background transition-all ${
                              isLatest
                          ...
                          <div className='flex flex-col sm:flex-row sm:items-start justify-between gap-1'>
                            <div>
                              ...
                              <p className='text-sm text-muted-foreground mt-0.5'>
                                {event.note ||
                                  `Status changed to ${event.statusChangedTo}`}
                              </p>
                            ...
                            <time
                              className={`text-xs font-mono whitespace-nowrap pt-1 ${
                                isLatest
          ...
          {proofImages.length > 0 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Evidence of transfer uploaded by admin
                </CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <CardContent className='pt-0'>
              {transaction.transferBankTransactionId ? (
                <DetailRow
                  ...
                  value={transaction.transferBankTransactionId}
                  copyable
                  mono
                />
              ) : (
              ...
              <DetailRow
                label='Provider'
                value={transaction.provider || 'Manual Transfer'}
              />
              <DetailRow
                label='Created'
              ...
              <CardContent className='bg-muted/30 pt-4 rounded-b-lg space-y-1'>
                ...
                <div className='pb-2'>
                  ...
                  <div className='flex items-center gap-2'>
                    ...
                    <CopyButton
                      text={transaction.withdrawBankAccountNumber}
      ...
      <Dialog open={cancelDialogOpen} onOpenChange={setCancelDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure? The funds (
              {formatCurrency(transaction.amount, transaction.currency)}) will
      ...
      <Dialog
        open={!!selectedProofImage}
        onOpenChange={(open) => !open && setSelectedProofImage(null)}
      >
        <DialogContent className='max-w-none w-100 h-auto p-0 backdrop-blur-sm bg-white/90 hover:bg-white border-none shadow-none z-[9999] flex flex-col justify-center items-center data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 duration-300'>
          {/* ACCESSIBILITY FIX: 
          ...
          */}
          <DialogTitle className='sr-only'>
            Proof of Transfer Image Preview
          </DialogTitle>
          <DialogDescription className='sr-only'>
            A full-screen view of the transfer proof image uploaded for this
            transaction.
          </DialogDescription>
