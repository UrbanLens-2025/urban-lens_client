...
export default function MyLocationRequestsPage() {
  ...
  const requestStats = useMemo(() => {
    ...
    const rejectedOrCancelled = locationRequests.filter(
      (req) =>
        req.status === 'REJECTED' || req.status === 'CANCELLED_BY_BUSINESS'
    ).length;
    ...
    <PageContainer>
      ...
      <div className='grid gap-6 md:grid-cols-2 lg:grid-cols-4'>
        ...
        <StatCard
          ...
          value={requestStats.awaitingReview.toLocaleString()}
          icon={Clock}
          color='amber'
          description='This page only'
        />

        <StatCard
          title='Approved'
          value={requestStats.approved.toLocaleString()}
          icon={CheckCircle}
          color='green'
          description='This page only'
        />
        ...
        <StatCard
          ...
          value={requestStats.rejectedOrCancelled.toLocaleString()}
          icon={XCircle}
          color='red'
          description='This page only'
        />
        ...
        <CardContent className='pt-4 pb-0 px-0'>
            ...
            <div className='flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between px-6 mb-4'>
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value='AWAITING_ADMIN_REVIEW'>
                    Pending Review
                  </SelectItem>
                  ...
                  <SelectItem value='CANCELLED_BY_BUSINESS'>
                    Cancelled
                  </SelectItem>
                </SelectContent>
              </Select>
          ...
          {locationRequests.length === 0 ? (
            <div className='flex flex-col items-center justify-center rounded-lg border border-dashed border-muted py-8 text-center mx-6 my-6'>
              ...
              <p className='mt-1.5 max-w-md text-xs text-muted-foreground'>
                Try adjusting your filters or submit a new location request to
                get started.
              </p>
                  ...
                  {locationRequests.map((request, index) => (
                    ...
                    >
                      <TableCell className='text-xs text-muted-foreground font-medium py-4 pl-6'>
                        {((requestsMeta?.currentPage ?? requestsPage) - 1) *
                          10 +
                          index +
                          1}
                      </TableCell>
          ...
          <div className='flex flex-col gap-2 border-t pt-3 pb-3 px-6 sm:flex-row sm:items-center sm:justify-between'>
            <div className='text-xs text-muted-foreground'>
              Page {requestsMeta?.currentPage ?? requestsPage} of{' '}
