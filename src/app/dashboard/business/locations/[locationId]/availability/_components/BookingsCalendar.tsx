    ...
    weekBookings.forEach((booking) => {
      ...
      booking.dates.forEach((dateSlot) => {
        ...
        let current = new Date(
          Math.max(start.getTime(), weekStartDate.getTime())
        );
        const bookingEnd = new Date(
          Math.min(end.getTime(), weekEndDate.getTime())
        );
        ...
        <div className='flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between'>
          <div className='flex-1'>
            ...
            <CardDescription className='text-base'>
              View and manage all bookings for this location. Click on booked
              slots to view details.
            </CardDescription>
          </div>
        </div>
      </CardHeader>
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        isDateToday &&
                          'bg-gradient-to-br from-primary/20 to-primary/10 border border-primary'
                      )}
                    >
                      <div
                        className={cn(
                  ...
                  return (
                    ...
                    >
                      ...
                      {DAYS_OF_WEEK.map((_, dayIndex) => {
                        ...
                        const statusColors = primaryBooking
                          ? getStatusColor(primaryBooking.status, bookingColor)
                          : null;
                        ...
                        const hasSameBookingBefore = prevBookings.some(
                          (b) => b.id === primaryBooking?.id
                        );
                        const hasSameBookingAfter = nextBookings.some(
                          (b) => b.id === primaryBooking?.id
                        );
                        const isContinuousStart =
                          hasBooking && !hasSameBookingBefore;
                        const isContinuousEnd =
                          hasBooking && !hasSameBookingAfter;
                        const isContinuousMiddle =
                          hasBooking &&
                          hasSameBookingBefore &&
                          hasSameBookingAfter;

                        return (
                          <div key={cellKey} className='h-[20px]'>
                            {hasBooking && primaryBooking ? (
                              bookings.length > 1 ? (
                                ...
                                <Popover
                                  open={isPopoverOpen}
                                  onOpenChange={(open) =>
                                    setOpenPopover(open ? cellKey : null)
                                  }
                                >
                                  <PopoverTrigger asChild>
                                    <div
                                      className={cn(
                                        ...
                                        bookings[0]?.bookingColor?.bg ||
                                          statusColors?.bg,
                                        bookings[0]?.bookingColor?.border ||
                                          statusColors?.border,
                                        statusColors?.hover,
                                        'text-white',
                                        'hover:shadow-md hover:scale-[1.08] hover:z-10',
                                        isPopoverOpen &&
                                          'ring-2 ring-offset-1 ring-primary z-20'
                                      )}
                                    >
                                ...
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <div
                                      onClick={() =>
                                        handleBookingClick(primaryBooking.id)
                                      }
                                      className={cn(
                                    ...
                                    >
                                      ...
                                      <div className='relative'>
                                        <span className='opacity-90 text-xs group-hover:opacity-0 transition-opacity'>
                                          ●
                                        </span>
                                  ...
                                  >
                                    <div className='space-y-1'>
                                      ...
                                      <div className='text-xs text-muted-foreground'>
                                        {format(primaryBooking.start, 'HH:mm')}{' '}
                                        - {format(primaryBooking.end, 'HH:mm')}
                                      </div>
                                      ...
                                      {bookingColor && (
                                        <div className='text-[10px] text-muted-foreground mt-1 pt-1 border-t'>
                                          Slots with the same corner indicator
        ...
        <div className='rounded-lg border border-primary/10 bg-gradient-to-r from-primary/5 via-primary/10 to-primary/5 p-3 shadow-sm'>
          <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2'>
                ...
                <div>
                  ...
                  <div className='text-xs text-muted-foreground mt-0.5'>
                    {weekBookings && weekBookings.length > 0 ? (
                      <>
                        {weekBookings.length} booking
                        {weekBookings.length !== 1 ? 's' : ''} •{' '}
                        {formatCurrency(stats.totalRevenue.toString())} revenue
                      </>
                    ) : (
            ...
            {weekBookings && weekBookings.length > 0 && (
              <Badge
                variant='secondary'
                className='text-xs font-bold px-2.5 py-1 bg-primary/10 text-primary border border-primary/20'
              >
                {weekBookings.length}
              </Badge>
            )}
