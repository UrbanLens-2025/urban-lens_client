...
const formatDateRange = (
  ...
  if (sortedDates.length > 0) {
    ...
    for (let i = 1; i < sortedDates.length; i++) {
      ...
      if (
        new Date(current.endDateTime).getTime() ===
...
function BookingCalendar({
  ...
  const getCellBooking = (day: Date, timeSlotHour: number) => {
    ...
    if (timeSlotHour === 22) {
      ...
      slotEnd = setMinutes(
        setHours(startOfDay(day), timeSlotHour + TIME_SLOT_HOURS),
        0
      );
    }
    ...
    if (timeSlotHour === 22 && !bottomHalfCovered) {
      ...
      for (const dateRange of dates) {
        ...
        const startsAt23 =
          getHours(bookingStart) === 23 && getMinutes(bookingStart) === 0;
        const endsAt00 =
          getHours(bookingEnd) === 0 && getMinutes(bookingEnd) === 0;
        const isOnCurrentDay =
          bookingStartTime >= dayStartTime && bookingStartTime < dayEndTime;
        const endsOnNextDay =
          bookingEndTime >= nextDayStartTime &&
          bookingEndTime < nextDayStartTime + 60 * 60 * 1000; // Within first hour of next day
...
function LocationCard({
  ...
  const averageRating = (displayLocation as LocationWithAnalytics)
    ?.averageRating;
  const totalReviews =
    (displayLocation as LocationWithAnalytics)?.totalReviews ?? 0;
  ...
  const businessEmail = (displayLocation as LocationWithAnalytics)?.business
    ?.email;
  const businessPhone = (displayLocation as LocationWithAnalytics)?.business
    ?.phone;
  const businessWebsite = (displayLocation as LocationWithAnalytics)?.business
    ?.website;
...
export default function EventLocationPage({
  ...
  const { data: detailedLocation } = useBookableLocationById(
    currentBooking?.location?.id || null
  );
            ...
            {hasCurrentBooking && currentBooking ? (
              <div className='space-y-6'>
                ...
                <div className='grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch'>
                                ...
                                <div className='space-y-3'>
                                  ...
                                  <div className='flex items-center justify-between'>
                                    ...
                                    <p className='text-sm font-semibold'>
                                      {formatCurrency(
                                        (
                                          Number(currentBooking.amountToPay) *
                                            (currentBooking as any)
                                              .systemCutPercentage || 0
                                        ).toString()
                                      )}
                                    </p>
                                  </div> */}
                  ...
                  {currentBooking?.id && (
                    <ReportBookingModal
                      open={isReportBookingModalOpen}
                      onOpenChange={setIsReportBookingModalOpen}
                      bookingId={currentBooking.id}
                    />
                  )}
