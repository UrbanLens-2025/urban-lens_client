...
function getStatusIcon(status: string) {
  ...
  if (statusUpper === "PENDING" || statusUpper === "PROCESSING" || statusUpper === "READY_FOR_PAYMENT") {
...
export default function AdminTransactionDetailPage({
  ...
  const { data: transaction, isLoading, isError } = useAdminExternalTransactionById(transactionId);
  ...
  const handleStartProcessing = () => {
    setConfirmDialog({
      ...
      description: "Are you sure you want to start processing this withdrawal transaction? This will move it to PROCESSING status.",
      onConfirm: () => {
  ...
  const handleMarkFailed = () => {
    ...
    setConfirmDialog({
      ...
      description: "Are you sure you want to mark this transfer as failed? This will move the transaction to TRANSFER_FAILED status and unlock the funds.",
      requiresReason: true,
      onConfirm: () => {}, // Will be handled by handleConfirmAction
    });
  };
  ...
  const handleReject = () => {
    ...
    setConfirmDialog({
      ...
      description: "Are you sure you want to reject this withdrawal transaction? This will move it to REJECTED status and unlock the funds.",
      requiresReason: true,
      onConfirm: () => {}, // Will be handled by handleConfirmAction
    });
  };
  ...
  const handleConfirmAction = () => {
    if (confirmDialog.action === "reject") {
      ...
      rejectTransaction.mutate({ transactionId, rejectionReason: currentReason });
      setConfirmDialog({ ...confirmDialog, open: false });
      setRejectionReason("");
      ...
      <div className="grid gap-6 lg:grid-cols-3">
            ...
            <CardContent>
              <div className="space-y-4">
                <div>
                  ...
                  <p className={`text-4xl font-bold ${isDeposit ? "text-green-600" : "text-orange-600"}`}>
                    {isDeposit ? "+" : "-"}
                ...
                <div className="flex items-center gap-2 pt-2 border-t">
                  ...
                  <div>
                    <p className="text-sm font-medium">Status: {transaction.status}</p>
                    <p className="text-xs text-muted-foreground">
                      {transaction.status === "PENDING" && "Awaiting processing"}
                      {transaction.status === "COMPLETED" && "Transaction completed successfully"}
                      {transaction.status === "TRANSFERRED" && "Funds transferred"}
                      {transaction.status === "TRANSFER_FAILED" && "Transfer failed"}
            ...
            <CardContent className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <p className="font-mono text-sm flex-1 truncate">{transaction.id}</p>
                    <Button
                      variant="ghost"
                      size="sm"
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <p className="font-mono text-sm flex-1 truncate">{transaction.walletId}</p>
                    <Button
                      variant="ghost"
                      size="sm"
              ...
              <CardContent className="space-y-4">
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    ...
                    <Badge variant="secondary" className="mt-1">{transaction.provider}</Badge>
                  </div>
                  {transaction.providerTransactionId && (
                    <div>
                      <p className="text-xs text-muted-foreground mb-1">Provider Transaction ID</p>
                      <div className="flex items-center gap-2">
                        <p className="font-mono text-sm flex-1 truncate">{transaction.providerTransactionId}</p>
                        <Button
                          variant="ghost"
                          size="sm"
                    ...
                    <div className="md:col-span-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          asChild
                        >
                          <a href={transaction.paymentUrl} target="_blank" rel="noreferrer">
              ...
              <CardContent className="space-y-4">
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    ...
                    <p className="text-sm font-medium">{transaction.withdrawBankName}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground mb-1">Account Number</p>
                    <div className="flex items-center gap-2">
                      <p className="font-mono text-sm flex-1">{transaction.withdrawBankAccountNumber}</p>
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => copyToClipboard(transaction.withdrawBankAccountNumber || "")}
                  ...
                  <div className="md:col-span-2">
                    ...
                    <p className="text-sm font-medium">{transaction.withdrawBankAccountName}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
                  ...
                  {transaction.timeline.map((event, index) => (
                    <div key={event.id} className="flex gap-4">
                      ...
                      <div className="flex-1 space-y-1 pb-4">
                        ...
                        <p className="text-sm font-semibold">{event.action.replace(/_/g, " ")}</p>
        ...
        <div className="space-y-6">
          ...
          {transaction.createdBy && (
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2 pt-4 border-t">
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <p className="font-mono text-xs flex-1 truncate">{transaction.createdBy.id}</p>
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => copyToClipboard(transaction.createdBy!.id)}
                        title="Copy User ID"
                      >
                  ...
                  {transaction.createdBy.phoneNumber && (
                    <div>
                      ...
                      <p className="text-sm font-medium">{transaction.createdBy.phoneNumber}</p>
                    </div>
                  )}
                  <div>
                    <p className="text-xs text-muted-foreground mb-1">Onboarding Status</p>
                    <Badge variant={transaction.createdBy.hasOnboarded ? "default" : "secondary"}>
                      {transaction.createdBy.hasOnboarded ? "Onboarded" : "Not Onboarded"}
                    </Badge>
                  </div>
                </div>
              </CardContent>
           ...
           {/* Admin Actions */}
           {isWithdraw && (canStartProcessing || canCompleteProcessing || canMarkFailed || canReject) && (
         ...
         <DialogContent className="sm:max-w-[500px]">
           ...
           {confirmDialog.requiresReason && (
             <div className="space-y-2 py-4">
               <Label htmlFor={confirmDialog.action === "reject" ? "rejectionReason" : "failureReason"}>
                 {confirmDialog.action === "reject" ? "Rejection Reason *" : "Failure Reason *"}
               </Label>
               <Textarea
                 id={confirmDialog.action === "reject" ? "rejectionReason" : "failureReason"}
                 placeholder={
                   confirmDialog.action === "reject"
                     ? "Please provide a reason for rejecting this transaction..."
                     : "Please provide a reason for why the transfer failed..."
                 }
                 value={confirmDialog.action === "reject" ? rejectionReason : failureReason}
                 onChange={(e) => {
                   if (confirmDialog.action === "reject") {
           ...
           <DialogFooter>
             ...
             <Button
               variant={confirmDialog.action === "reject" || confirmDialog.action === "failed" ? "destructive" : "default"}
