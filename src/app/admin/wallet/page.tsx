...
const getDirectionIcon = (direction: string) => {
  ...
  if (dir === "DEPOSIT") {
    return <Download className="h-4 w-4 text-green-600 dark:text-green-400" />;
  }
...
const getDirectionColor = (direction: string) => {
  const dir = direction.toUpperCase();
  return dir === "DEPOSIT" ? "text-green-600 dark:text-green-400" : "text-orange-600 dark:text-orange-400";
};
...
export default function AdminWalletPage() {
  ...
  const { data: transactionsData, isLoading: isLoadingTransactions, isError: isTransactionsError } = useAdminExternalTransactions({
    page: currentPage,
    limit: itemsPerPage,
    sortBy,
  });
  ...
  const stats = useMemo(() => {
    return {
      totalDeposits: transactions
        .filter((t: WalletExternalTransaction) => t.direction.toUpperCase() === "DEPOSIT")
        .reduce((sum: number, t: WalletExternalTransaction) => sum + parseFloat(t.amount), 0),
      totalWithdrawals: transactions
        .filter((t: WalletExternalTransaction) => t.direction.toUpperCase() === "WITHDRAW")
        .reduce((sum: number, t: WalletExternalTransaction) => sum + parseFloat(t.amount), 0),
      pendingCount: transactions.filter((t: WalletExternalTransaction) => t.status.toUpperCase() === "PENDING").length,
      completedCount: transactions.filter((t: WalletExternalTransaction) => t.status.toUpperCase() === "COMPLETED").length,
    };
  }, [transactions]);
  ...
  return (
    <PageContainer maxWidth="7xl">
      ...
      <PageHeader
        ...
        description="Monitor and manage system wallets and transactions"
        icon={Wallet}
        action={
      ...
      {!isLoading && (escrowWallet || systemWallet) && (
        <Card className="border-2 border-primary/10 shadow-xl bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950 dark:to-indigo-950 border-blue-200 dark:border-blue-800">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-muted/50">
                        ...
                        <TableHead className="font-semibold">Bank Info</TableHead>
