...
const getStatusBadge = (status: string) => {
  ...
  const variants: Record<string, { variant: "default" | "secondary" | "destructive" | "outline"; label: string; className?: string }> = {
    COMPLETED: { variant: "default", label: "Completed", className: "bg-green-100 text-green-800 border-green-200 dark:bg-green-900/30 dark:text-green-400 dark:border-green-800" },
    PENDING: { variant: "secondary", label: "Pending", className: "bg-yellow-100 text-yellow-800 border-yellow-200 dark:bg-yellow-900/30 dark:text-yellow-400 dark:border-yellow-800" },
    READY_FOR_PAYMENT: { variant: "secondary", label: "Ready for Payment", className: "bg-blue-100 text-blue-800 border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800" },
    PROCESSING: { variant: "secondary", label: "Processing", className: "bg-yellow-100 text-yellow-800 border-yellow-200 dark:bg-yellow-900/30 dark:text-yellow-400 dark:border-yellow-800" },
    TRANSFERRED: { variant: "default", label: "Transferred", className: "bg-green-100 text-green-800 border-green-200 dark:bg-green-900/30 dark:text-green-400 dark:border-green-800" },
    TRANSFER_FAILED: { variant: "destructive", label: "Transfer Failed", className: "bg-red-100 text-red-800 border-red-200 dark:bg-red-900/30 dark:text-red-400 dark:border-red-800" },
    REJECTED: { variant: "destructive", label: "Rejected", className: "bg-red-100 text-red-800 border-red-200 dark:bg-red-900/30 dark:text-red-400 dark:border-red-800" },
    CANCELLED: { variant: "outline", label: "Cancelled", className: "bg-gray-100 text-gray-800 border-gray-200 dark:bg-gray-900/30 dark:text-gray-400 dark:border-gray-800" },
  };
...
export default function AdminWithdrawRequestsPage() {
  ...
  // Dialog states
  const [selectedTransaction, setSelectedTransaction] = useState<WalletExternalTransaction | null>(null);
  ...
  const { data: transactionsData, isLoading, isFetching } = useAdminExternalTransactions({
    page,
    limit: itemsPerPage,
  ...
  const withdrawTransactions = useMemo(() => {
    return allTransactions.filter((t: WalletExternalTransaction) => 
  ...
  const filteredTransactions = useMemo(() => {
    return withdrawTransactions.filter((t: WalletExternalTransaction) => {
      ...
      if (statusFilter === 'pending') {
        if (!['PENDING', 'READY_FOR_PAYMENT'].includes(statusUpper)) return false;
  ...
  const stats = useMemo(() => {
    const pending = withdrawTransactions.filter((t: WalletExternalTransaction) => 
      ['PENDING', 'READY_FOR_PAYMENT'].includes(t.status.toUpperCase())
    ).length;
    const processing = withdrawTransactions.filter((t: WalletExternalTransaction) => 
      t.status.toUpperCase() === 'PROCESSING'
    ).length;
    const totalAmount = withdrawTransactions
      .filter((t: WalletExternalTransaction) => 
        ['PENDING', 'READY_FOR_PAYMENT', 'PROCESSING'].includes(t.status.toUpperCase())
      )
      .reduce((sum: number, t: WalletExternalTransaction) => sum + parseFloat(t.amount), 0);
    const completed = withdrawTransactions.filter((t: WalletExternalTransaction) => 
  ...
  const handleConfirmAction = () => {
    ...
    if (actionDialog.action === 'start') {
      ...
      rejectTransaction.mutate(
        ...
        {
          onSuccess: () => {
            setActionDialog({ open: false, action: 'reject', transaction: null });
            setRejectionReason('');
            queryClient.invalidateQueries({ queryKey: ['adminExternalTransactions'] });
          },
        }
      ...
      markFailed.mutate(
        ...
        {
          onSuccess: () => {
            setActionDialog({ open: false, action: 'failed', transaction: null });
            setFailureReason('');
            queryClient.invalidateQueries({ queryKey: ['adminExternalTransactions'] });
          },
        }
      ...
      completeProcessing.mutate(
        ...
        {
          onSuccess: () => {
            setActionDialog({ open: false, action: 'complete', transaction: null });
            setProofOfTransferUrl(undefined);
            setTransferBankTransactionId('');
            queryClient.invalidateQueries({ queryKey: ['adminExternalTransactions'] });
          },
  ...
  const isActionLoading = startProcessing.isPending || completeProcessing.isPending || markFailed.isPending || rejectTransaction.isPending;

  return (
    <PageContainer maxWidth="7xl">
      <PageHeader
        title="Withdraw Requests"
        description="Process and manage withdrawal requests from users"
        icon={Upload}
        action={
      ...
      <Card className="border-2 border-primary/10 shadow-xl bg-card/80 backdrop-blur-sm">
        <CardHeader className="pb-4 border-b border-primary/10">
          ...
          <CardDescription>
            {isLoading ? 'Loading...' : `Showing ${filteredTransactions.length} withdraw request${filteredTransactions.length !== 1 ? 's' : ''}`}
          </CardDescription>
        </CardHeader>
        <CardContent className="pt-6">
          {isLoading ? (
            ...
            <div className="space-y-4">
              {filteredTransactions.map((transaction: WalletExternalTransaction) => {
                ...
                const canStartProcessing = statusUpper === 'PENDING' || statusUpper === 'READY_FOR_PAYMENT';
                const canCompleteProcessing = statusUpper === 'PROCESSING';
                const canMarkFailed = statusUpper === 'PROCESSING';
                const canReject = statusUpper === 'PENDING' || statusUpper === 'READY_FOR_PAYMENT';
                            ...
                            <div className="pt-4 border-t border-primary/10 space-y-2">
                              ...
                              <div className="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                  ...
                                  <p className="font-medium">{transaction.withdrawBankName}</p>
                                </div>
                                <div>
                          ...
                          <div className="flex flex-col gap-2">
                            {canStartProcessing && (
                              <Button
                                ...
                                onClick={() => handleStartProcessing(transaction)}
                            ...
                            {canCompleteProcessing && (
                              <Button
                                ...
                                onClick={() => handleCompleteProcessing(transaction)}
                                disabled={isActionLoading}
                              >
            ...
            <div className="flex items-center justify-between mt-6 pt-4 border-t border-primary/10">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={filteredTransactions.length < itemsPerPage || isLoading}
                  className="border-2 border-primary/20 hover:border-primary/40"
                >
                  Next
                </Button>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                This reason will be recorded in the transaction history and visible to the user.
              </p>
            </div>
          </div>
          <DialogFooter>
      ...
      >
        <DialogContent className="sm:max-w-[500px]">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="transferBankTransactionId">Transfer Transaction ID *</Label>
              <Input
