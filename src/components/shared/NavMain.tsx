...
export function NavMain({
  ...
  const isItemActive = (url: string) => {
    ...
    const isExcludedChild = excludedChildRoutes.some(
      (excluded) => pathname.includes(excluded) && !url.includes(excluded)
    );
            ...
            return (
              <SidebarMenuItem key={item.title}>
                <SidebarMenuButton
                  ...
                  isActive={isActive}
                  className={cn(
                    'relative transition-all duration-200 ease-out rounded-lg mx-1',
                    'before:absolute before:left-0 before:top-1/2 before:-translate-y-1/2 before:w-1.5 before:h-8 before:rounded-r-full before:transition-all before:duration-200',
                    'group-data-[collapsible=icon]:before:hidden group-data-[collapsible=icon]:justify-center',
                    isActive
                      ? 'bg-sidebar-accent text-sidebar-accent-foreground font-semibold shadow-md shadow-sidebar-primary/20 ring-2 ring-sidebar-primary/20 before:bg-sidebar-primary before:h-10 before:w-1.5'
                  ...
                  {item.icon && (
                    <item.icon
                      className={cn(
                        ...
                        isActive
                          ? 'text-sidebar-primary scale-110 drop-shadow-sm'
